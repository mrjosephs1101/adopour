  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL || `${window.location.origin}/feed`,
        },
      })
      if (error) throw error
    } catch (error: unknown) {
      ...
      <div className="w-full max-w-sm">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">Connect with friends and the world</p>
          </div>
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <form onSubmit={handleLogin}>
                ...
                <div className="mt-4 text-center text-sm">
                  ...
                  <Link href="/auth/sign-up" className="underline underline-offset-4 text-cyan-600">
                    Sign up
                  </Link>
                </div>
              </form>
